...
def evaluate_csv(csv_path: Path, output_path: Path, model_path: Path = MODEL_PATH):
    ...
    fieldnames = ["id", "category", "injection_text", "predicted_label", "predicted_label_name", "injection_prob"]
    ...
    print("-" * 65)
    for cat, stats in sorted(category_stats.items(), key=lambda x: -(x[1]["detected"]/x[1]["total"])):
